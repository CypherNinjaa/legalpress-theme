...
function legalpress_scripts()
{
    ...
    // Mobile Bottom Navigation (only if enabled)
    if (get_theme_mod('legalpress_mobile_bottom_nav', true)) {
        wp_enqueue_style('legalpress-mobile-nav', LEGALPRESS_URI . '/assets/css/mobile-bottom-nav.css', array('legalpress-lawchakra'), LEGALPRESS_VERSION);
        wp_enqueue_script('legalpress-mobile-nav', LEGALPRESS_URI . '/assets/js/mobile-bottom-nav.js', array(), LEGALPRESS_VERSION, true);
        
        // Localize search data for mobile nav
        wp_localize_script('legalpress-mobile-nav', 'legalpressSearch', array(
 ...
 * Allows clients to update the theme directly from GitHub
 */
require_once LEGALPRESS_DIR . '/inc/github-updater.php';
...
/**
 * Get Category Color
 * Returns a color based on category slug for consistent styling
 *
 * @since 2.5.0
 * @param string $slug Category slug
 * @return string Hex color code
 */
function legalpress_get_category_color($slug) {
    // Predefined colors for common legal categories
    $colors = array(
        'supreme-court' => '#1e3a5f',
        'high-court' => '#059669',
        'legal-updates' => '#dc2626',
        'opinion' => '#7c3aed',
        'news' => '#0891b2',
        'judgment' => '#d97706',
        'case-law' => '#4f46e5',
    ...
    // Generate a consistent color from slug
    $hash = md5($slug);
    $hue = hexdec(substr($hash, 0, 2)) + hexdec(substr($hash, 2, 2));
    $hue = $hue % 360;
    
    // Use HSL to get a nice color
...
/**
 * Get Social Icon SVG by platform name
 *
 * @since 2.5.0
 * @param string $platform Platform name (twitter, facebook, etc.)
 * @return string SVG icon markup
 */
function legalpress_get_social_icon($platform) {
    $icons = array(
